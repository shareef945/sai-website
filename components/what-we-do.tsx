...
export default function WhatWeDo() {
  ...
  useEffect(() => {
    ...
    const drawSemicircle = () => {
      ...
      // Replace the boxContents array with dynamically generated content from services
      const boxContents = services.slice(0, 5).map((service, index) => {
        let position;
        ...
        // Split the title into two parts at a logical point
      ...
      boxContents.forEach((box) => {
        ...
        ctx.fillText(
          box.title,
          pos.x + boxWidth / 2,
          pos.y + boxHeight / 2 - (box.subtitle ? 10 : 0)
        );
  ...
  return (
    <div className="w-full md:h-screen flex items-center justify-center overflow-hidden">
      {isMobile ? (
        <div className="w-full max-w-lg px-4 py-8  mt-12 flex flex-col space-y-4">
          ...
          {services.map((item, index) => (
            ...
            >
              <p className="text-white text-center">
                {/* {item.title} */}
                {item.title && (
                  <span className="block mt-2">{item.title}</span>
                )}
              </p>
            </div>
          ))}
